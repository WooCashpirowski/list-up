...
export function useListItems(userId: string) {
    ...
    useEffect(() => {
        ...
        void saveCachedCollection(userId, 'list-items', items).catch(
            (nextError) => {
                setError(getErrorMessage(nextError));
            },
        );
    ...
    useEffect(() => {
        const completed = items
            ...
            .map((item) => ({
                item,
                expiresAt:
                    new Date(item.done_at!).getTime() + DONE_RETENTION_MS,
            }));

        if (completed.length === 0) return;

        const nextExpiry = Math.min(
            ...completed.map(({ expiresAt }) => expiresAt),
        );
    ...
    const unassignCategoryItems = useCallback(
        async (categoryId: string, name: string): Promise<boolean> => {
            const normalizedName = name.trim().toLocaleLowerCase('pl');
            const matchingItems = items.filter(
                (item) =>
                    item.category_id === categoryId &&
                    item.name.trim().toLocaleLowerCase('pl') === normalizedName,
            );

            for (const previous of matchingItems) {
                setItems((current) =>
                    current.map((item) =>
                        item.id === previous.id
                ...
                try {
                    await executeOrQueueMutation(
                        ...
                        () =>
                            gateway.updateListItem(previous.id, {
                                category_id: null,
                            }),
                    );
                } catch (nextError) {
    ...
    const clearItems = useCallback(
        async (listId: string, onlyDone = false): Promise<void> => {
            const removed = items.filter(
                (item) =>
                    item.list_id === listId && (!onlyDone || item.is_done),
            );
