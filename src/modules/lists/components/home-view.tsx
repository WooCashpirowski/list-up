...
export function HomeView({
  ...
  const counts = useMemo(() => {
    ...
    for (const item of items) {
      const current = result.get(item.list_id) ?? { total: 0, remaining: 0 }
      current.total += 1
      ...
      <div className="flex flex-col gap-3">
            ...
            >
              <div className="flex items-center gap-2">
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-0.5 block text-sm text-muted-foreground">
                      {count.total === 0
                        ? mounted
                          ? t('home.emptyUpdated', {
                              time: timeAgo(list.updated_at, locale),
                            })
                          : t('home.empty')
                        : mounted
                          ? t('home.itemsLeftUpdated', {
                ...
                <button
                  ...
                  aria-label={t('home.rename', { title: list.title })}
                ...
                <button
                  onClick={() => {
                    if (window.confirm(t('home.deleteConfirm', { title: list.title }))) {
                      void onDeleteList(list.id)
                    }
                  }}
                  aria-label={t('home.delete', { title: list.title })}
              ...
              {isEditing && (
                <div className="mt-3 flex gap-2 border-t border-border/70 pt-3">
                  <input
                    ...
                    onKeyDown={(event) => {
                      if (event.key === 'Enter') void submitRename(list.id)
                      if (event.key === 'Escape') setEditingId(null)
                    }}
                    aria-label={t('home.newName', { title: list.title })}
        ...
        {lists.length === 0 && (
          <div className="rounded-3xl border border-dashed border-border px-5 py-10 text-center">
            ...
            <p className="mt-3 font-semibold">{t('home.noLists')}</p>
          ...
          >
            ...
            <span>
              ...
              <span className="mt-0.5 flex items-center gap-1 text-sm text-muted-foreground">
                <ListChecks className="size-3.5" /> {t('home.createListDescription')}
              </span>
            </span>
          </button>
        )}
