...
function AuthenticatedApp({ userId }: { userId: string }) {
    ...
    const handleRemoveCategoryItem = async (
        categoryId: string,
        name: string,
    ) => {
        ...
        const unassigned = await itemsState.unassignCategoryItems(
            categoryId,
            name,
        );
        ...
        const keywords = category.keywords.filter(
            (keyword) =>
                keyword.trim().toLocaleLowerCase('pl') !== normalizedName,
        );
        if (keywords.length === category.keywords.length) return true;

        return categoriesState.updateCategory(categoryId, { keywords });
    };

    useEffect(() => {
        if (!listsState.isLoading && openListId && !openList)
            replaceWithLists();
